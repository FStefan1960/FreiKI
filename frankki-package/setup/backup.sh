...
BACKUP_DIR="/tmp/frankki-backup-${BACKUP_DATE}"
BACKUP_FILE="/tmp/frankki-backup-${BACKUP_DATE}.tar.gz"
GONEO_USER="114598f88090u6"
GONEO_PASS="E7r7l7e3n1w1e4g"
GONEO_HOST="450881.test-my-website.de"
GONEO_DIR="frankki"
SSH_KEY="/home/frank/.ssh/id_goneo_backup"
STACK_DIR="/home/frank/freiki-package"

echo "=== FrankKI Backup ${BACKUP_DATE} ==="
...
docker exec PostgreSQL pg_dumpall -U frankki_user --no-role-passwords | gzip > "${BACKUP_DIR}/postgres-dumpall.sql.gz"
...
echo "-> Packen..."
tar czf "${BACKUP_FILE}" -C /tmp "frankki-backup-${BACKUP_DATE}"
