...
app.get('/api/modes', async (req, res) => {
  ...
  if (session?.uid && !isAdmin) {
    try {
      ...
      const r = await client.query('SELECT use_areas, use_paperless FROM freiki_users WHERE id=$1', [session.uid]);
      client.release();
      if (r.rows[0]) {
        liveUse = r.rows[0].use_areas || [];
        livePaperless = !!r.rows[0].use_paperless;
      }
...
app.post('/api/admin/users', async (req, res) => {
  if (!adminSession(req)) return res.status(403).json({ error: 'Nur für Administratoren' });
  const { username, role, use, manage, first_name, last_name, funktion, email, use_paperless } = req.body || {};
  ...
  try {
    ...
    const { rows } = await pgPool.query(
      ...
      [username.trim(), hash, r, first_name||'', last_name||'', funktion||'', email||'', cleanAreas(use), cleanAreas(manage), !!use_paperless]);
    if (autoGenerated && email) {
      try { await sendWelcomeMail(email.trim(), username.trim(), password, first_name||'', last_name||''); }
...
app.post('/api/admin/users/:id/resend-welcome', async (req, res) => {
  if (!adminSession(req)) return res.status(403).json({ error: 'Nur für Administratoren' });
  try {
    const { rows } = await pgPool.query('SELECT username, email, first_name, last_name FROM freiki_users WHERE id=$1', [parseInt(req.params.id,10)]);
