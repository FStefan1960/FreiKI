...
async function sendWelcomeMail(to, username, password) {
  ...
  const handbuchPath = path.join(__dirname, `${brandConfig.name}_Benutzerhandbuch.pdf`);
  const attachments = fs.existsSync(handbuchPath)
    ? [{ filename: `${brandConfig.name}_Benutzerhandbuch.pdf`, path: handbuchPath }]
    : [];
...
app.get('/api/modes', async (req, res) => {
  ...
  if (session?.uid && !isAdmin) {
    try {
      ...
      const r = await client.query('SELECT use_areas FROM freiki_users WHERE id=$1', [session.uid]);
      client.release();
  ...
  const hasPaperless = isAdmin || userAreas.includes('paperless');
...
app.post('/api/change-password', async (req, res) => {
  ...
  try {
    const { rows } = await pgPool.query('SELECT * FROM freiki_users WHERE id=$1', [s.uid]);
    const u = rows[0];
    ...
    await pgPool.query('UPDATE freiki_users SET password_hash=$1, updated_at=now() WHERE id=$2', [hash, u.id]);
  ...
  try {
    const { rows } = await pgPool.query(
      `SELECT id, username, role, first_name, last_name, funktion, email,
              use_areas, manage_areas, suspended FROM freiki_users ORDER BY username`);
    const users = rows.map(u => ({
      ...
      use: u.use_areas || [], manage: u.manage_areas || [],
    }));
    res.json({ users });
...
app.post('/api/admin/users', async (req, res) => {
  if (!adminSession(req)) return res.status(403).json({ error: 'Nur für Administratoren' });
  const { username, role, use, manage, first_name, last_name, funktion, email } = req.body || {};
  let password = (req.body || {}).password;
  if (!isValidUsername(username)) return res.status(400).json({ error: 'Benutzername: 3–64 Zeichen, nur Buchstaben, Zahlen und ._-' });
  ...
  try {
    ...
    const { rows } = await pgPool.query(
      `INSERT INTO freiki_users (username,password_hash,role,first_name,last_name,funktion,email,use_areas,manage_areas)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`,
      [username.trim(), hash, r, first_name||'', last_name||'', funktion||'', email||'', cleanAreas(use), cleanAreas(manage)]);
...
app.post('/api/admin/users/:id', async (req, res) => {
  ...
  const { role, use, manage, suspended, first_name, last_name, funktion, email } = req.body || {};
  if (email && !isValidEmail(email)) return res.status(400).json({ error: 'Ungültige E-Mail-Adresse' });
  const r = VALID_ROLES.includes(role) ? role : 'default';
  ...
  try {
    ...
    const { rowCount } = await pgPool.query(`UPDATE freiki_users SET ${fields.join(',')} WHERE id=$1`, vals);
...
app.post('/api/admin/users/:id/password', async (req, res) => {
  ...
  try {
    const hash = await bcrypt.hash(password, 10);
    const { rowCount } = await pgPool.query('UPDATE freiki_users SET password_hash=$1, updated_at=now() WHERE id=$2', [hash, parseInt(req.params.id,10)]);
    if (!rowCount) return res.status(404).json({ error: 'Nutzer nicht gefunden' });
    res.json({ ok: true });
  } catch (e) { console.error('admin/users password:', e.message); res.status(500).json({ error: 'Fehlgeschlagen' }); }
});

// Willkommensmail erneut senden (setzt Passwort zurück)
app.post('/api/admin/users/:id/resend-welcome', async (req, res) => {
  if (!adminSession(req)) return res.status(403).json({ error: 'Nur für Administratoren' });
  try {
    const { rows } = await pgPool.query('SELECT username, email FROM freiki_users WHERE id=$1', [parseInt(req.params.id,10)]);
    if (!rows.length) return res.status(404).json({ error: 'Nutzer nicht gefunden' });
    const u = rows[0];
    if (!u.email) return res.status(400).json({ error: 'Keine E-Mail-Adresse hinterlegt' });
    ...
    await sendWelcomeMail(u.email, u.username, newPassword);
    res.json({ ok: true });
...
app.delete('/api/admin/users/:id', async (req, res) => {
  ...
  try {
    const { rowCount } = await pgPool.query('DELETE FROM freiki_users WHERE id=$1', [id]);
    if (!rowCount) return res.status(404).json({ error: 'Nutzer nicht gefunden' });
    res.json({ ok: true });
...
app.get('/api/me', async (req, res) => {
  ...
  if (!s) return res.status(401).json({ error: 'Nicht angemeldet' });
  try {
    const { rows } = await pgPool.query('SELECT username, email, role, first_name, last_name FROM freiki_users WHERE id=$1', [s.uid]);
...
app.post('/api/transcribe', uploadAudio.single('audio'), async (req, res) => {
  ...
  let email = '';
  try {
    const { rows } = await pgPool.query('SELECT email FROM freiki_users WHERE id=$1', [s.uid]);
