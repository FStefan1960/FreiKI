...
BACKUP_ROOT="/home/frank/backups"
BACKUP_DIR="${BACKUP_ROOT}/frankki-${BACKUP_DATE}"
BACKUP_FILE="${BACKUP_ROOT}/frankki-backup-${BACKUP_DATE}.tar.zst"
LOG_FILE="/home/frank/frankki-backup.log"
MAX_BACKUPS=7
STACK_DIR="/home/frank/freiki-package"
NOTIFY_EMAIL="admin@fst60.de"
...
HIDRIVE_KEY="/home/frank/.ssh/hidrive_backup_key"
HIDRIVE_SFTP_HOST="sftp.hidrive.ionos.com"
HIDRIVE_RSYNC_HOST="rsync.hidrive.ionos.com"
HIDRIVE_DIR="users/freiki-admin/backups/frankki"
...
notify() {
  ...
  # curl kann den EHLO-Hostnamen nicht überschreiben; der Server-Hostname "fst60" (kein FQDN)
...
cleanup_remote() {
  ...
  old_files=$(echo "$listing" | grep -E '^frankki-backup-.*\.tar\.zst$' | sort | head -n -"${MAX_BACKUPS}" || true)
  if [ -z "$old_files" ]; then
...
# --- Backup ---
log "=== FrankKI Backup ${BACKUP_DATE} ==="
mkdir -p "$BACKUP_DIR"
START_TIME=$(date +%s)
...
VOLUMES=(
  ...
  # keine Nutzerdaten). Kein Mattermost/Hermes/lokales vLLM auf FrankKI (nutzt Mistral-API).
)
...
log "-> Packen..."
if ! tar c -C "$BACKUP_ROOT" "frankki-${BACKUP_DATE}" | zstd -T0 -19 -o "$BACKUP_FILE"; then
...
if [ "${#FAILURES[@]}" -eq 0 ]; then
  ...
  notify "✅ FrankKI Backup Erfolgreich" "FrankKI Backup ${BACKUP_DATE} abgeschlossen (Dauer: ${DURATION}s)."
else
  log "=== Backup FEHLGESCHLAGEN (Dauer: ${DURATION}s): ${FAILURES[*]} ==="
  notify "❌ FrankKI Backup Fehlgeschlagen" "FrankKI Backup ${BACKUP_DATE} hatte Fehler: ${FAILURES[*]}. Prüfe ${LOG_FILE}."
  exit 1
fi
